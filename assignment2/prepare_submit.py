...
def check_file(file: str, check_aid: str):
    ...
    with zipfile.ZipFile(file, 'r') as zz:
        ...
        for info in zz.infolist():
            ...
            assert info.filename.startswith(target_prefix), \
                'There should only be one top-level dir (with your campus id as the dir-name) inside the zip file.'
    ...
    print(f"Read zipfile {file}, please check that your campus-id is: {target_prefix[:-1]}")
